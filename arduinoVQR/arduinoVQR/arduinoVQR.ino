...
// Allocate grayscale buffer in PSRAM (External RAM) to prevent ESP32 from crashing
uint8_t *grayscale_buf = nullptr;

// Variable to remember the last scanned QR code so the screen doesn't flicker
int last_painting_id = -1;
...
void setup() {
  ...
  // Allocate memory for QR scanner IN PSRAM (MALLOC_CAP_SPIRAM)
  grayscale_buf = (uint8_t *)heap_caps_malloc(320 * 240, MALLOC_CAP_SPIRAM);
  if (!grayscale_buf) {
    Serial.println("Failed to allocate grayscale buffer in PSRAM");
  }
...
const unsigned long SCAN_INTERVAL = 5000;  // Scan once every 5 seconds
...
void scanQRCode(camera_fb_t *fb) {
  ...
  if (count > 0) {
    ...
    if (quirc_decode(&code, &data) == QUIRC_SUCCESS) {
      ...
      // Only update the screen if it's a new QR code (prevents flickering)
      if (paintingID != last_painting_id) {
        last_painting_id = paintingID;
